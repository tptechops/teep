...
jQuery(function($){
 ...
 $('#customloginform').on('submit',function(e){
		...
		if(rcres.length){
			grecaptcha.reset();
			$('.submit_custom_login').val('Wait...');
			$('.submit_custom_reg1').val('Wait...');
			...
			$.ajax({
				...
				success: function(msg){
					...
					if(msg==1){
						//alert('Login Success!!');
						$('.rerror').html('<p style="color:green">Login Success!!</p>');
						setTimeout(function () {
							window.location.href = $('#redirect_url').val();
						}, 3000);
						
					}else if(msg == 2){
						$('.rerror').html('<p style="color:red">The username/password you entered is incorrect.</p>');
						setTimeout(function () {
							$('.rerror').empty('');
						}, 3000);
					}else if(msg == 3){
						...
						$('.rerror').html('<p style="color:red">The security token mis matched.</p>');
						setTimeout(function () {
							$('.rerror').empty('');
						}, 3000);
					}
					...
					if(data.responseText.includes('html')){
						...
						$('.sign-in-form').html(data.responseText);
						//$('.sign-in-form').find(['target="_blank"']).remove();
					}else{
...
/*
		...
		if (rcres.length) {
			...
			$.ajax({
				...
				success: function (msg) {
					...
					$('.submit_custom_login').val('Login');
					if (msg == 1) {
						$('.rerror').html('<p style="color:green">Please enter OTP sent to your email for verification.</p>');
						$('#custom-user, #custom_pass, #remember-me').hide();
						$('#otp_verification').show();
						$('.submit_custom_login').val('Verify OTP');
						$('#customloginform').attr('action', my_ajax_object.otp_verify_url); // Replace with OTP verification URL
					} else if (msg == 2 || msg == 3 || msg == 4) {
						// Handle other login error messages as before
						$('.rerror').html('<p style="color:red">Login failed. Please try again.</p>');
					}
	...
	*/
	...
	/* $('#customregform').on('submit', function(e) {
        ...
        formData.append('action', 'edit_reg_data'); // Add the action name
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // Perform actions after successful submission (e.g., show success message, redirect, etc.)
            },
            error: function(errorThrown) {
                // Handle the error
                console.log('Error:', errorThrown);
                // Perform actions if there's an error (e.g., display error message)
            }
        });
    });
*/
			...
			$.ajax({
				...
				//	console.log(url);
					...
					if(msg==1){
						...
						$('.rerror').html('<p style="color:red">You reched the limit of max allowed otp.Try after 30 minutes later.</p>');
						setTimeout(function () {
							$('.rerror').empty('');
						}, 3000);
					}
			...
			$.ajax({
				...
				success: function(msg){
					...
					if(msg==1){
						...
						$('.rerror').html('<p style="color:red">You reched the limit of max allowed requests.Try after 30 minutes later.</p>');
						setTimeout(function () {
							$('.rerror').empty('');
						}, 3000);
					}else{
