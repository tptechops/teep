<?php
/* Template Name: Principal Dashboard */
session_start();
get_header();
global $wpdb;
    ...
    <section class="section bg-e4f5ff">
        <div class="container">
                ...
                <div class="col-md-6 col-12">
                    <div class="dash-col user-info">
                        ...
                        <div class="update-area">
                            <form name="form" action="<?php echo home_url(); ?>/principal-dashboard/" method="post">
                                    ...
                                    <div class="dash-event-inn">
                                        ...
                                        <div class="de-content-wrap">
                                            ...
                                            <div class="btn-wrap event-btn-wrap">
                                                ...
                                                <a href="#" class="def-btn btn-3 event-btn">VIEW</a>
            ...
            <div class="progress-area">
                <h3 class="fs-30 text-upppercase mb-5">MY STUDENT PROGRESS</h3>
                <?php
					$tablename = 'student_events';
					$results = $wpdb->get_results( "SELECT * FROM $tablename WHERE role='student'");
                ...
                <div class="progress-table-wrap">
                    <p>
                        Your students yet to participate in any event
                    </p>
                </div>
                <?php }
                    ...
                    <table class="progress-table">
                        ...
                        <tbody>
                            <?php 
								...
								$results = $wpdb->get_results( "SELECT * FROM $tablename WHERE school_name='".$schoolName."' AND role='student'"); // Query to fetch data from database table and storing in $results
						
								foreach($results as $row){   
								    $participantId = $row->post_id;
									$postArray[] = $participantId;
									$array = array_unique($postArray);
								}
								foreach($array as $row){
									
									$participantId = $row;
									
									$totalParticipant = $wpdb->get_var( "SELECT COUNT(*) FROM $tablename WHERE school_name='".$schoolName."' AND role='student' AND post_id='".$participantId."'"); 
									$totalAttend = $wpdb->get_var( "SELECT COUNT(*) FROM $tablename WHERE school_name='".$schoolName."' AND role='student' AND participated=1 AND post_id='".$participantId."'");
									$totalWinner = $wpdb->get_var( "SELECT COUNT(*) FROM $tablename WHERE school_name='".$schoolName."' AND role='student' AND status=1 AND post_id='".$participantId."'");
			...
			<!-- My Teachers PROGRESS -->
            <div class="progress-area">
                <h3 class="fs-30 text-upppercase mb-5">MY TEACHERS PROGRESS</h3>
                <?php
					$tablename = 'student_events';
					$results = $wpdb->get_results( "SELECT * FROM $tablename WHERE role='teacher'");
					$praticipateCount = count($results);
					if($praticipateCount == 0){
                    ...
                    <table class="progress-table">
                        ...
                        <tbody>
                            <?php 
								...
								$results = $wpdb->get_results( "SELECT * FROM $tablename WHERE school_name='".$schoolName."' AND role='teacher'"); // Query to fetch data from database table and storing in $results
						
								foreach($results as $row){   
								    $participantId = $row->post_id;
									$postArray1[] = $participantId;
									$array1 = array_unique($postArray1);
								}
								foreach($array1 as $row){
									
									$participantId = $row;
									
									$totalParticipant = $wpdb->get_var( "SELECT COUNT(*) FROM $tablename WHERE school_name='".$schoolName."' AND role='teacher' AND post_id='".$participantId."'"); 
									$totalAttend = $wpdb->get_var( "SELECT COUNT(*) FROM $tablename WHERE school_name='".$schoolName."' AND role='teacher' AND participated=1 AND post_id='".$participantId."'");
									$totalWinner = $wpdb->get_var( "SELECT COUNT(*) FROM $tablename WHERE school_name='".$schoolName."' AND role='teacher' AND status=1 AND post_id='".$participantId."'");
								    $content_post = get_post_field('post_content', $participantId);
								    $title = get_post_field('post_title', $participantId);
							?>
                            <tr>
			...
			<!-- My School PROGRESS -->
            <div class="progress-area">
                <h3 class="fs-30 text-upppercase mb-5">MY SCHOOL PROGRESS</h3>
                <?php
					$tablename = 'student_events';
					$results = $wpdb->get_results( "SELECT * FROM $tablename WHERE role='teacher' OR role='student'");
					$praticipateCount = count($results);
					if($praticipateCount == 0){
                    ...
                    <table class="progress-table">
                        ...
                        <tbody>
                            <?php 
								...
								$results = $wpdb->get_results( "SELECT * FROM $tablename WHERE school_name='".$schoolName."' AND role='teacher' OR role='student'"); // Query to fetch data from database table and storing in $results
						
								foreach($results as $row){   
								    $participantId = $row->post_id;
									$postArray2[] = $participantId;
									$array2 = array_unique($postArray2);
								}
								foreach($array2 as $row){
									
									$participantId = $row;
									
									$totalTeacherParticipant = $wpdb->get_var( "SELECT COUNT(*) FROM $tablename WHERE school_name='".$schoolName."' AND role='teacher' AND post_id='".$participantId."'"); 
									$totalStudentParticipant = $wpdb->get_var( "SELECT COUNT(*) FROM $tablename WHERE school_name='".$schoolName."' AND role='student' AND post_id='".$participantId."'"); 
									$totalTeacherAttend = $wpdb->get_var( "SELECT COUNT(*) FROM $tablename WHERE school_name='".$schoolName."' AND role='teacher' AND participated=1 AND post_id='".$participantId."'");
									$totalStudentAttend = $wpdb->get_var( "SELECT COUNT(*) FROM $tablename WHERE school_name='".$schoolName."' AND role='student' AND participated=1 AND post_id='".$participantId."'");
									$totalTeacherWinner = $wpdb->get_var( "SELECT COUNT(*) FROM $tablename WHERE school_name='".$schoolName."' AND role='teacher' AND status=1 AND post_id='".$participantId."'");
									$totalStudentWinner = $wpdb->get_var( "SELECT COUNT(*) FROM $tablename WHERE school_name='".$schoolName."' AND role='student' AND status=1 AND post_id='".$participantId."'");
								    $content_post = get_post_field('post_content', $participantId);
								    $title = get_post_field('post_title', $participantId);
							?>
                            <tr>
                                <td><?php echo $title; ?></td>
                                <td><?php echo $totalTeacherParticipant+$totalStudentParticipant; ?></td>
                                <td><?php echo $totalTeacherAttend+$totalStudentAttend; ?></td>
                                <td><?php echo $totalTeacherWinner+$totalStudentWinner; ?></td>
                            </tr>
                            <?php
								}
							?>
