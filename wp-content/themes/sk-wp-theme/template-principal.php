<?php 
	/* Template Name: My Account Principal Page */ 
	get_header(); 
	
	$number   = 10;
	$paged    = (get_query_var('paged')) ? get_query_var('paged') : 1;
	$offset   = ($paged - 1) * $number;
	$users    = get_users( array( 'role' => 'principal' ));
	$query    = get_users('&role=principal&orderby=ID&order=desc&offset='.$offset.'&number='.$number);
			  ...
			  <div class="row mb-3">
				...
				<div class="col-md-6 col-sm-12 " style="text-align:right">
				  <div class="add-btn text-end" style="display: inline-flex;">
					<a style="margin-right:10px;" href="<?=home_url()?>/my-account/add-principal"><button type="button" class="custom-btn">Add Principal</button></a>
					<a href="<?=home_url()?>/my-account/students?principal_export_csv=true"><button type="button" class="custom-btn exportbtn">Export All</button></a>
				</div>
				</div>
				<div class="col-12">
					<form>
						...
						<table class="table">
							...
							?>
							  <tr>
								...
								<td><a href="<?=home_url()?>/my-account/edit-principal?key=<?=base64_encode($q->ID)?>"><i class="fas fa-edit"></i></a>
								<a class="confirmIt text-danger mr-2" href="javascript:void(0)" data-href="<?=home_url()?>/my-account/principals?key=<?=base64_encode($q->ID)?>&del=true&pa=<?=get_query_var('paged')?>&pastr=principals"><i class="fas fa-trash"></i></a></td>
							  </tr>
								<?php $i++;} } ?>
