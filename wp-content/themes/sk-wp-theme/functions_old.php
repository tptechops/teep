...
// function check_otp_requests_limit() {
// 	// Set the time window for OTP requests (in seconds)
// 	$time_window = 3600; // 1 hour

// 	// Get the user's IP address
// 	$user_ip = $_SERVER['REMOTE_ADDR'];

// 	// Check if the user has requested an OTP within the defined time window
// 	$otp_requests = get_transient('otp_requests_' . $user_ip);

// 	if ($otp_requests >= 5) { // Limiting to 5 OTP requests per time window
// 		// Handle the case where the user has exceeded the limit
// 		// You can log the attempt, display an error message, or take other actions as needed
// 		echo 'You have exceeded the limit for OTP requests. Please try again later.';
// 		exit;
// 	} else {
// 		// Increment the count of OTP requests for this user's IP address
// 		$otp_requests = ($otp_requests === false) ? 1 : $otp_requests + 1;
// 		set_transient('otp_requests_' . $user_ip, $otp_requests, $time_window);
// 	}
// }
...
if( function_exists('acf_add_options_page') ) {
	...
	acf_add_options_page(array(
		...
		'capability' => 'check_otp_requests_limit_posts',
		'redirect'  => false
	));
...
function get_login_data() {
	//$testLoginObject = new Limit_Login_Attempts();
        ...
        if( $user_info ) {
            ...
            //if( wp_check_password( $custom_pass, $user_info->user_pass, $user_id ) ) {
				...
				*/
				...
				if ( is_wp_error( $user ) ) {
					if(isset($user->errors['too_many_tried'])){
...
function set_reg_data() {
	...
	// check_otp_requests_limit();
	if( get_option('val-'.$oEmail) ){
		$currentTime = date('Y-m-d H:i:s');
		$lastSentTime = $chkotp['sent_time'];
		$totalOtpSent = $chkotp['total_sent'];
		$to_time = strtotime($currentTime);
		$from_time = strtotime($lastSentTime);
		$minutes = round(abs($to_time - $from_time) / 60,2);
		if($minutes<=30 && $totalOtpSent>=5){
			echo json_encode(11);die;
		}
	}
	if( isset($_SESSION['val-'.$oEmail]) ){
		$currentTime = date('Y-m-d H:i:s');
		$lastSentTime = $_SESSION['val-'.$oEmail]['sent_time'];
		$totalOtpSent = $_SESSION['val-'.$oEmail]['total_sent'];
		$to_time = strtotime($currentTime);
		$from_time = strtotime($lastSentTime);
		$minutes = round(abs($to_time - $from_time) / 60,2);
		if($minutes<=30 && $totalOtpSent>=5){
    ...
    if(!$captcha){
 	...
 	$response = json_decode(file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=6LdLiUAnAAAAADI-7xDdGbwnvNSad1ZTo8ERBPdr&response=".$captcha."&remoteip=".$_SERVER['REMOTE_ADDR']), true);
    if($response['success'] == false)
    {
		echo json_encode(5);die;
        //echo '<h2>You are spammer ! Get the @$%K out</h2>';die;
    }
    
	
    if( wp_verify_nonce( (isset($_POST['custom_register_nonce']) ? $_POST['custom_register_nonce'] : ''), 'custom-register-nonce' ) ) {
        /*$uernameexists = username_exists( $_POST['custom_user'] );
		if($uernameexists){
			echo json_encode(3);die;
		}*/
		...
		$body = '<p>Hi, '.$_SESSION['form_data']['custom_fname'].' <br>Your OTP to complete verification process is <br> <b>'.$_SESSION['otp'].'</b></p></br>Thanks,<br> Team TEEP';
		$headers = array('Content-Type: text/html; charset=UTF-8');

		wp_mail( $to, $subject, $body, $headers );
		$_SESSION['totalotpsent'] = 0;
		if( isset($_SESSION['val-'.$to])  ){
			$totalSent = $_SESSION['val-'.$to]['total_sent'] + 1;
		}else{
			$totalSent = 1; 
		}
		
		if( isset($chkotp['total_sent']) ){
			$totalSent = $chkotp['total_sent'] + 1;
		}else{
			$totalSent = 1; 
		}
		...
		$arr = array('sent_time' => date('Y-m-d H:i:s') , 'total_sent' => $totalSent );
		if(get_option('val-'.$to)){
			update_option('val-'.$to, $arr);
		} else {
			add_option('val-'.$to, $arr);
		}
		#print_r($_SESSION);die;
		echo json_encode(1);die;
		/*$user_id = wp_insert_user( array(
		  'user_login' => $_POST['custom_user'],
...
function match_otp_data(){
	...
	if( wp_verify_nonce( (isset($_POST['custom_register_nonce']) ? $_POST['custom_register_nonce'] : ''), 'custom-register-nonce' ) ) {
		...
		if( get_option('wrongotp-'.$to) ){
			$currentTime = date('Y-m-d H:i:s');
			$lastTryTime = $chkotp['try_time'];
			$totalOtpTry = $chkotp['total_try'];
			$to_time = strtotime($currentTime);
			$from_time = strtotime($lastTryTime);
			$minutes = round(abs($to_time - $from_time) / 60,2);
		...
		if( $_SESSION['otp'] == $_POST['opt-value']){
			...
			if ( is_wp_error( $user_id  ) ) {
				...
				add_user_meta( $user_id, 'gender', $_SESSION['form_data']['gender']);
				add_user_meta( $user_id, '_gender', 'field_63ea3e6837c5c');
				
				add_user_meta( $user_id, 'profile_pic', $_SESSION['profile_id']);
				add_user_meta( $user_id, '_profile_pic', 'field_63ea49e462170');
				
				add_user_meta( $user_id, 'school', $_SESSION['form_data']['school_id']);
				add_user_meta( $user_id, 'school_name', get_the_title($_SESSION['form_data']['school_id']));
				add_user_meta( $user_id, '_school', 'field_63ea3d3a37c57');
				
				add_user_meta( $user_id, 'class_id', $_SESSION['form_data']['class_id']);
				add_user_meta( $user_id, '_class_id', 'field_63ea3dd737c58');
				
				add_user_meta( $user_id, 'section', $_SESSION['form_data']['section']);
				add_user_meta( $user_id, '_section', 'field_63ea3dfe37c59');
				
				add_user_meta( $user_id, 'contact_number', $_SESSION['form_data']['phone_number']);
				add_user_meta( $user_id, '_contact_number', 'field_63ea3e4337c5a');
				
				add_user_meta( $user_id, 'alternate_contact_number', $_SESSION['form_data']['alter_phone_number']);
				add_user_meta( $user_id, '_alternate_contact_number', 'field_63ea3e4e37c5b');
				
				add_user_meta( $user_id, 'other_school_name', $_SESSION['form_data']['other_school_name']);
				
				//student id generate school_uid
				if( $_SESSION['form_data']['register_as'] == 'teacher'){
					$str = 'T';
				}elseif( $_SESSION['form_data']['register_as'] == 'principal' ){
					$str = 'P';
				}else{
					$str = 'S';
				}
			...
			if( isset($chkotp['total_try']) ){
				$total_try = $chkotp['total_try'] + 1;
			}else{
				$total_try = 1; 
			}
...
function resend_otp(){
	...
	if( get_option('resend-'.$to) ){
		$currentTime = date('Y-m-d H:i:s');
		$lastTryTime = $chkotp['resend_time'];
		$totalOtpTry = $chkotp['total_resend'];
		$to_time = strtotime($currentTime);
		$from_time = strtotime($lastTryTime);
		$minutes = round(abs($to_time - $from_time) / 60,2);
		if($minutes<=30 && $totalOtpTry>=5){
	...
	$totalOtpSent = $_SESSION['totalotpsent'];
	$curDate = date('Y-m-d H:i:s');
	
	$start = (isset($_SESSION['sentTime'])) ? strtotime($_SESSION['sentTime']) : 1;
	...
	if( isset($chkotp['total_resend']) ){
		$total_resend = $chkotp['total_resend'] + 1;
	}else{
		$total_resend = 1; 
	}
...
add_action( 'wp_ajax_nopriv_check_otp_requests_limit_reg_data', 'check_otp_requests_limit_reg_data' );
...
function edit_reg_data() {
	...
	if ($otpAttempts >= $maxAttempts) {
		...
		if (!isset($_SESSION['otp_blocked'][$userKey]) || $_SESSION['otp_blocked'][$userKey] < $currentTime) {
			// Block the IP for 30 minutes
			$_SESSION['otp_blocked'][$userKey] = $currentTime + $timeFrameSeconds;
		}
	...
	if (isset($_SESSION['otp_blocked'][$userKey]) && $_SESSION['otp_blocked'][$userKey] > $currentTime) {
		$blockTimeLeft = $_SESSION['otp_blocked'][$userKey] - $currentTime;
		echo json_encode(array('status' => 12, 'time_left' => $blockTimeLeft)); // IP blocked, return time left
		die;
	}
