...
function get_login_data() {
    ...
    if (!isset($_SESSION[$otp_attempts_key])) {
        $_SESSION[$otp_attempts_key] = array(
            'attempts' => 0,
            'cooldown_start' => 0
        );
    }

    $otp_attempts = $_SESSION[$otp_attempts_key]['attempts'];
    $cooldown_start = $_SESSION[$otp_attempts_key]['cooldown_start'];
    $cooldown_duration = 1800; // Cooldown period in seconds (30 minutes)

    if ($otp_attempts >= $otp_attempt_limit && time() < $cooldown_start + $cooldown_duration) {
        // Exceeded OTP attempt limit and still within cooldown period
        echo json_encode(12); // Response indicating cooldown period in effect
        die;
    }
    ...
    if (isset($_POST['custom_user']) && isset($_POST['custom_pass'])) {
        ...
        if (wp_verify_nonce((isset($_POST['custom_login_nonce']) ? $_POST['custom_login_nonce'] : ''), 'custom-login-nonce')) {
            ...
            if ($user_info && wp_check_password($custom_pass, $user_info->user_pass, $user_info->ID)) {
                ...
                $_SESSION[$otp_attempts_key]['attempts']++;

                echo json_encode(5); // Response indicating OTP sent
                die;
            } else {
                // Increment OTP attempts for this IP even if authentication fails
                $_SESSION[$otp_attempts_key]['attempts']++;

                // Check if the cooldown period should be started
                if ($_SESSION[$otp_attempts_key]['attempts'] >= $otp_attempt_limit) {
                    $_SESSION[$otp_attempts_key]['cooldown_start'] = time();
                }
...
function resend_otp_login() {
    ...
    $otp_resend_limit = 5; // Limit OTP resending to 5 times
    $otp_resend_time_window = 30 * 60; // Time window for OTP resending (30 minutes)
    ...
    // Check if OTP resend attempts have exceeded the limit within the time window
    if ($_SESSION[$otp_attempts_key] >= $otp_resend_limit) {
        // Set IP block expiration time to 30 minutes from now
        $_SESSION['otp_blocked_login'][$ip] = time() + $otp_resend_time_window;
    ...
    // Continue with OTP sending if IP is not blocked and resend attempts are within limit
...
function set_reg_data() {
    ...
    if ($_POST['captcha'] != 4) {
        echo json_encode(15); // Incorrect answer to the math problem
        die;
    }

    // Rate Limit OTP Resend Attempts
        $otp_resend_limit = 5; // Limit OTP resending to 5 times
        $otp_resend_time_window = 30 * MINUTE_IN_SECONDS; // Time window for OTP resending (30 minutes)

        $otp_resend_attempts = isset($chkotp['resend_attempts']) ? $chkotp['resend_attempts'] : 0;
        $last_resend_time = isset($chkotp['last_resend_time']) ? strtotime($chkotp['last_resend_time']) : 0;
        $current_time = time();
        $time_since_last_resend = $current_time - $last_resend_time;

        // Check if OTP resend attempts have exceeded the limit within the time window
        if ($last_resend_time && $time_since_last_resend < $otp_resend_time_window && $otp_resend_attempts >= $otp_resend_limit) {
            echo json_encode(14); // Exceeded OTP resend limit
            die;
        }
    ...
    $body = '<p>Hi, ' . $_SESSION['form_data']['custom_fname'] . ' <br>Your OTP to complete the verification process is <br> <b>' . $otp . '</b></p></br>Thanks,<br> Team TEEP';
    $headers = array('Content-Type: text/html; charset=UTF-8');

    wp_mail($to, $subject, $body, $headers);

    $otp_resend_attempts++;
    $_SESSION['val-' . $to]['resend_attempts'] = $otp_resend_attempts;
    $_SESSION['val-' . $to]['last_resend_time'] = date('Y-m-d H:i:s');
    update_option('val-' . $to, array('resend_attempts' => $otp_resend_attempts, 'last_resend_time' => date('Y-m-d H:i:s')));
...
function resend_otp() {
    ...
    if ($chkotp && isset($chkotp['resend_time'])) {
        $last_resend_time = strtotime($chkotp['resend_time']);
        $total_resend_attempts = isset($chkotp['total_resend']) ? $chkotp['total_resend'] : 0;
        $minutes_since_last_resend = round((time() - $last_resend_time) / 60);

        if ($minutes_since_last_resend <= $otp_resend_time_window && $total_resend_attempts >= $otp_resend_limit) {
            echo json_encode(2); // Rate limit exceeded
            die;
        }

        // Clear resend attempts if the last attempt was made more than 30 minutes ago
        if ($minutes_since_last_resend > $otp_resend_time_window) {
    ...
    $resend_data = array('resend_time' => date('Y-m-d H:i:s'), 'total_resend' => $total_resend);
    update_option('resend-' . $to, $resend_data);
...
function add_reg_data() {
        ...
        if (wp_verify_nonce((isset($_POST['custom_register_nonce']) ? $_POST['custom_register_nonce'] : ''), 'custom-register-nonce')) {
            ...
            $otp_resend_limit = 5; // Maximum OTP resend attempts allowed
            $otp_resend_time_window = 30 * 60; // Time window for OTP resending (30 minutes)
    
            $otp_resend_attempts_key = 'otp_resend_attempts_' . $_POST['email'];
            $otp_resend_attempts = get_option($otp_resend_attempts_key, 0);
            $last_resend_time = get_option('last_resend_time_' . $_POST['email'], 0);
            $current_time = time();
    
            // Check if the last resend attempt was made within the time window
            if (($current_time - $last_resend_time) < $otp_resend_time_window) {
                // Increment resend attempt count
                $otp_resend_attempts++;
    
                // Check if the resend attempt limit has been exceeded
                if ($otp_resend_attempts > $otp_resend_limit) {
                    echo json_encode(11); // Rate limit exceeded
                    die;
                }
            } else {
                // Reset resend attempt count if the time window has elapsed
                $otp_resend_attempts = 1;
            }
    
            // Update resend attempt count and time
            update_option($otp_resend_attempts_key, $otp_resend_attempts);
            update_option('last_resend_time_' . $_POST['email'], $current_time);

            // Clear resend attempt count and time if the user successfully registers
            update_option($otp_resend_attempts_key, 0);
            update_option('last_resend_time_' . $_POST['email'], 0);
