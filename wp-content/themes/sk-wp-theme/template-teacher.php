<?php 
	/* Template Name: My Account Teacher Page */  
	get_header(); 
	...
	$query    = get_users('&role=teacher&orderby=ID&order=desc&offset='.$offset.'&number='.$number);
			  ...
			  <div class="row mb-3">
				...
				<div class="col-md-6 col-sm-12 " style="text-align:right">
				  <div class="add-btn text-end" style="display: inline-flex;">
					<a style="margin-right:10px;" href="<?=home_url()?>/my-account/add-teacher"><button type="button" class="custom-btn">Add Teacher</button></a>
					<a href="<?=home_url()?>/my-account/students?teacher_export_csv=true"><button type="button" class="custom-btn exportbtn">Export All</button></a>
				</div>
				</div>
				<div class="col-12">
					<form>
						...
						<table class="table">
							...
							?>
							  <tr>
								...
								<td><a href="<?=home_url()?>/my-account/edit-teacher?key=<?=base64_encode($q->ID)?>"><i class="fas fa-edit"></i></a>
								<a class="confirmIt text-danger mr-2" href="javascript:void(0)" data-href="<?=home_url()?>/my-account/teachers?key=<?=base64_encode($q->ID)?>&del=true&pa=<?=get_query_var('paged')?>&pastr=teachers"><i class="fas fa-trash"></i></a></td>
							  </tr>
								<?php $i++;} } ?>
